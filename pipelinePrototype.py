...
def detect_persons_robust(image):
    # Single-pass fast YOLO inference
    res = model.predict(image, classes=[0], device=DEVICE, verbose=False, conf=0.2, imgsz=640)[0]
...
def process_video(source_path, output_path=None, display=False):
    ...
    print(f"Video FPS: {fps:.1f} | Running YOLO+RTMPose at every {infer_every_n} frames (~{fps/infer_every_n:.1f} inferences/sec)")
